...
} from "@/lib/actions/adminJobApplication.actions";
import { connectToDB } from "@/lib/mongodb";
import {
  AdminJobResumeFormSchema,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  type AdminJobResumeFormData,
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const validatedData = AdminJobResumeFormSchema.parse(body);
...
export async function PUT(request: Request) {
  try {
    ...
    try {
      const validatedData = AdminJobResumeFormSchema.partial().parse(body);
